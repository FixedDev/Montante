...
public class Binary extends Operation {
    ...
    public static boolean isAddition(Node op) {
        ...
        if (getLeft() instanceof Fraction && isCommutative()) {
            ...
            if (r1 instanceof Fraction) {
                ...
                Optional<Binary> reduced = pairs.stream()
                        .filter(p -> p[1] > 1)
                        .map(p -> Operation.exp(p[0], mult(p[1], r2)))
                        .reduce(Node::mult);
                ...
                if (reduced.isPresent()) {
                    ...
                    if (retained.isPresent()) { //TODO: debug negative
                        return Operation.mult(exp(retained.get() * (r1Negative ? -1 : 1), r2), reduced.get()).simplify();
                    } else {
            ...
            if (r2 instanceof Fraction) {
                ...
                if (v2 > 1) {
                    long t = (long) r2.val();
                    Operation o1 = exp(r1, new RawValue(t));
                    Operation o2 = exp(r1, Operation.sub(r2, new RawValue(t)));
                    ...
                    switch (uop.getName()) {
                        case "log": // (10^n)^log(b) = b^n
                            if (getLeft() instanceof RawValue) {
                                ...
                                if (r.isInteger() && isInteger(n)) {
                                    return exp(uop.getOperand(), new RawValue(n)).simplify();
                                }
                            }
                            break;
                        case "ln": // (e^a)^ln(b) = b^a
                            ...
                            if (k instanceof RawValue) {
                                ...
                                if (r1.isInteger()) {
                                    return exp(uop.getOperand(), r1).simplify();
                                }
                            }
                    }
            }
