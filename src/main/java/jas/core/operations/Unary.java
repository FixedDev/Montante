...
public class Unary extends Operation implements BinLeafNode {
    ...
    public static void define(String name, String expression) {
        define(name, Compiler.compile(expression));
    }
                        ...
                        if (factors.size() > 1) {
                            ...
                            if (allTheSame(num)) {
                                ...
                                if (!r1.equals(r)) // do not return 1*ln(10), or it would cause StackOverflow error!
                                    return mult(new RawValue(num[0]), new Unary(r1, operation));
                                else return this;
                            }
                        }
                }
...
//                case "csc":
        ...
        switch (operation.getName()) {
            ...
            case "sec":
                o = Operation.div(this.getOperand(), div(Constants.get("pi"), TWO)).simplify();
                if (o instanceof RawValue && ((RawValue) o).isInteger()) {
                    return abs(((RawValue) o).longValue() % 2) == 1;
                }
                break;
            case "cot": // domain: x != n*pi
            case "csc":
                o = div(this.getOperand(), Constants.get("pi")).simplify();
                if (o instanceof RawValue && ((RawValue) o).isInteger())
                    return true;
